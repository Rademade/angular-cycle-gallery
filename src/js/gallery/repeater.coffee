angular.module('multiGallery').directive 'galleryRepeater', [
  ...
  (GalleryRenderer, ItemsStorage, MoverHolder, GalleryMover, MoverTouch, GalleryEvents, Resize, $window, $rootScope, ResizeEmulator)->
    ...
    link: ($scope, $element, $attr, nullController, renderFunction) ->
      ...
      _$holder.on 'touchstart', (e)-> touch.touchStart(e.originalEvent.touches[0].pageX)
      _$body.on 'touchend', (e)-> touch.touchEnd()
      _$body.on 'touchmove', (e)-> touch.touchMove(e.originalEvent.touches[0].pageX)


      # Methods

      $rootScope.setGalleryIndex = (index)-> mover.setIndex( index - 0 )
      $rootScope.updateGallerySizes = -> mover.updateSizes()
